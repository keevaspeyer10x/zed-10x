...
test("comparison only judges matched lanes and ignores sparse unrelated groups", () => {
  ...
  const cleanComparison = buildComparison([
    ...credibleGroup("zed", "local", "1.11.3", "acp.complete"),
    ...credibleGroup("zed10x", "local", "20260727.1", "acp.complete"),
  ]);
  assert.equal(cleanComparison.verdict, "no_material_difference");
  assert.equal(cleanComparison.comparisons[0].verdict, "no_material_difference");
});
