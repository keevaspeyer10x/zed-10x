...
test("launcher starts with no arguments", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zed-10x-launcher-empty-"));
  ...
  const result = spawnSync(path.join(macos, "zed-10x-launcher"), [], {
    env: {
      ...process.env,
      HOME: fakeHome,
      ZED_FAKE_RESULT: resultPath,
